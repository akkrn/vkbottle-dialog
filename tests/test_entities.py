...
def test_context_identity():
    ...
    c2 = Context(
        intent_id=c1.intent_id,
        stack_key=c1.stack_key,
        state=SG.a,
        start_data=None,
        dialog_data={"changed": True},
        widget_data={},
    )
    assert c1.same(c2)
...
def test_event_context_chat_detection():
    ev = EventContext(
        group_id=1, peer_id=2_000_000_005, owner_id=77, user_id=77, kind="message_event", raw=None
    )
    assert ev.is_chat and ev.stack_key == "vkd:stack:1:2000000005:77:0"
    ls = EventContext(
        group_id=1, peer_id=55, owner_id=55, user_id=55, kind="message_new", raw=None
    )
