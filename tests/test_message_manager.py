...
def new_msg(text="t", kb='{"inline":true,"buttons":[]}', kind=KeyboardKind.INLINE,
            mode=ShowMode.AUTO):
    return NewMessage(peer_id=5, text=text, keyboard=kb, keyboard_kind=kind,
                      attachments=[], show_mode=mode)
...
async def test_callback_edits_fresh_window(fake_api):
    mm = MessageManager(fake_api)
    stack = fresh_stack(last_cmid=50, last_message_sent_at=NOW - 100,
...
async def test_old_window_sends_new(fake_api):
    mm = MessageManager(fake_api)
    stack = fresh_stack(last_cmid=50, last_message_sent_at=NOW - 90_000,
                        last_keyboard_kind=KeyboardKind.INLINE)
    await mm.show_message(new_msg(), stack, trigger="message_event", now=NOW)
...
async def test_explicit_delete_and_send(fake_api):
    ...
    stack = fresh_stack(last_cmid=50, last_message_sent_at=NOW - 100,
                        last_keyboard_kind=KeyboardKind.INLINE)
    await mm.show_message(new_msg(mode=ShowMode.DELETE_AND_SEND), stack,
                          trigger="message_event", now=NOW)
