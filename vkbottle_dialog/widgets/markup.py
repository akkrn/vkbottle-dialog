...
def _button_doc(button: VKButton, action_type: str, intent_id: str,
                secret: str | None) -> dict:
    if button.action == "open_link":
        return {"action": {"type": "open_link", "link": button.link,
                           "label": _label(button)}}
...
def _validate(raw: RawKeyboard, max_buttons: int, max_rows: int) -> None:
    ...
    if problems:
        raise DialogConfigError(
            "клавиатура превышает лимиты VK: " + ", ".join(problems)
            + " — используйте ScrollingGroup/пагинацию"
        )


class InlineKeyboardFactory:
    def render(self, raw: RawKeyboard, intent_id: str,
               secret: str | None) -> RenderedKeyboard:
        if not raw:
            return RenderedKeyboard(json=None, kind=KeyboardKind.NONE)
        _validate(raw, INLINE_MAX_BUTTONS, INLINE_MAX_ROWS)
        buttons = [[_button_doc(b, "callback", intent_id, secret) for b in row]
                   for row in raw]
...
class TextKeyboardFactory:
    ...
    def render(self, raw: RawKeyboard, intent_id: str,
        ...
        buttons = [[_button_doc(b, "text", intent_id, secret) for b in row]
                   for row in raw]
