...
- [ ] С вашим id в `ADMIN_IDS` в беседе открываются все секции, **кроме** «Нижней клавиатуры»
  (ожидаемо падение с `DialogConfigError` — reply-клавиатура общая на весь чат, см. «Ограничения
  v0.2»); «🎠 Карусель» в беседе тоже использует callback-нижнюю клавиатуру — работает так же, как
  в ЛС.
