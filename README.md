...
Календарь, Счётчик, Мультивиджет, Мастер, Нижняя клавиатура, VK-фишки, ListGroup, Карусель,
...
- **«🎠 Карусель»** (`bot_dialogs/carousel_demo.py`) — 3-элементная VK-карусель товаров
  ...
  целиком; нижняя навигация — отдельная callback-клавиатура (`TextKeyboardFactory`).
...
- [ ] В беседе открываются все секции, **кроме** «Нижней клавиатуры» (ожидаемо падение с
  `DialogConfigError` — reply-клавиатура общая на весь чат, см. «Ограничения v0.2»); «🎠 Карусель»
  в беседе тоже использует callback-нижнюю клавиатуру — работает так же, как в ЛС.
...
- [ ] «🔒 Доступ»: добавьте бота в тестовую беседу, впишите свой VK user_id в `ADMIN_IDS`
  (`bot_dialogs/access_demo.py`) — под этим id клики по диалогу работают, под любым другим —
  тихо игнорируются (без снекбара, без ошибки в клиенте).
